...
void WorldModel::printJoints() {
  ...
  logWorlModel << "\n\n\tBall :" << ball.getPolarPos().getRo() << " "
      << ball.getPolarPos().getYaw() << " " << ball.getPolarPos().getPhi()
      << endl;
  logWorlModel << "Direction of head from flag :" << getDOF(DOF_HEAD_PAN)
      << endl;
  ...
  // log the Joint
  for (int i = 0; i < 22; i++) {
    logWorlModel << hingeJointsArray[i].getName() << " "
        << hingeJointsArray[i].getAxis() << ",";
  }
...
void WorldModel::setVelOfBall() {
  if (getDeltaTime() != 0)
    velOfBall = ((getGlobalPosOfBall() - getLastGlobalPosOfBall())
        / getDeltaTime());
  else
    return;
}
