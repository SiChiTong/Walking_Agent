...
void MessageParser::parseMessage(const std::string& msg)
{
    ...
    if (exp==NULL) return; else if (exp->list==NULL) return;
    else if (exp->list->next==NULL) return;
...
Point* MessageParser::parsePolarCoords(sexp_t* exp_p) {
    ...
    if(exp_p->ty != SEXP_LIST) {
        cerr << "ERROR parsing polar coordinates: s-expression is not a list. Returning (0,0,0). ";
        return new Point(0,0,0);
    }
    // the head of that list must be "pol"
    if(strcmp(exp_p->list->val,"pol")) {
        cerr << "ERROR parsing polar coordinates: \"pol\" identifier expected. Found " << (exp_p->list->val) << ". Returning (0,0,0). ";
        return new Point(0,0,0);
    }
